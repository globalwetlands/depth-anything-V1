#!/usr/bin/env python3
"""
Data Quality Checker for DIODE Dataset - Host Version
Runs directly on host system without Docker
"""
...
class DataQualityChecker:
    ...
    def load_sample_paths(self):
        ...
        if self.splits_dir and os.path.exists(self.splits_dir):
            print(f"📁 Loading samples from splits directory: {self.splits_dir}")
                    ...
                    with open(split_file, 'r') as f:
                        for line in f:
                            ...
                            if len(parts) >= 2:
                                ...
                                img_path = parts[0].replace('/data/', self.data_root + '/')
                                depth_path = parts[1].replace('/data/', self.data_root + '/')
            ...
            print(f"📁 Scanning directory for all samples: {self.data_root}")
    ...
    def run_quality_check(self):
        ...
        # Debug: Show first few samples with path conversion
        print(f"🔍 Sample paths (first 3):")
        for i, sample in enumerate(samples[:3]):
            print(f"  {i+1}. Image: {sample['image']}")
            print(f"     Depth: {sample['depth']}")
            print(f"     Mask:  {sample['mask']}")
            print(f"     Exists: img={os.path.exists(sample['image'])}, depth={os.path.exists(sample['depth'])}, mask={os.path.exists(sample['mask'])}")
            print()
...
def main():
    print("🔍 DIODE Dataset Quality Checker - Host Version")
    print("=" * 60)
    
    # Use host paths
    data_root = "/mnt/fast-data/monocular/data/diode-tnc"
    splits_dir = "./dataset_splits"  # Local splits directory
    ...
    if not os.path.exists(data_root):
        ...
        print("Make sure your data path is correct")
        return 1
    ...
    try:
        ...
        print(f"📊 Check the reports in: {output_dir}")
        return 0
    except Exception as e:
